...
describe('Input', () => {
  it('基本展示', () => {
    ...
    // classes
    expect(wrapper.classes()).toContain('vm-input--small')
    expect(wrapper.classes()).toContain('is-prepend')
    // should render input
    expect(wrapper.find('input').exists()).toBeTruthy()
    expect(wrapper.get('input').attributes('type')).toBe('text')
    // slots
    expect(wrapper.find('.vm-input__prepend').exists()).toBeTruthy()
    expect(wrapper.get('.vm-input__prepend').text()).toBe('prepend')
    expect(wrapper.find('.vm-input__prefix').exists()).toBeTruthy()
    expect(wrapper.get('.vm-input__prefix').text()).toBe('prefix')
  ...
  it('支持点击清空字符串', async () => {
    ...
    // 不出现对应的 Icon 区域
    expect(wrapper.find('.vm-input__clear').exists()).toBeFalsy()
    const input = wrapper.get('input')
    await input.trigger('focus')
    expect(wrapper.emitted()).toHaveProperty('focus')
    //  出现 Icon 区域
    expect(wrapper.find('.vm-input__clear').exists()).toBeTruthy()
    // 点击值变为空并且消失
    await wrapper.get('.vm-input__clear').trigger('click')
  ...
  it('支持切换密码显示', async () => {
    ...
    expect(wrapper.find('.vm-input__password').exists()).toBeFalsy()
    const input = wrapper.get('input')
    expect(input.element.type).toBe('password')
    //  出现 Icon 区域，并且 Icon 为特点的图标
    await input.setValue('123')
    const eyeIcon = wrapper.find('.vm-input__password')
